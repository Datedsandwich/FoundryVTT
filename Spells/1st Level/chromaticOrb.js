if (args[0].tag === 'OnUse') {
    ...
    const damageRoll = args[0].isCritical
        ? await new Roll(`${numDice}d8 + ${numDice * 8}`).evaluate({ async: true })  // This is how crits are handled in my game
        : await new Roll(`${numDice}d8`).evaluate({ async: true })

    new MidiQOL.DamageOnlyWorkflow(caster, casterToken, damageRoll.total, damageType, [target], damageRoll, {
