if (args[0].tag === "OnUse") {
    ...
    async function applyDivineWord(target) {
        if(['celestial', 'elemental', 'fey', 'fiend'].some(type => (target.actor.data.data.details?.type?.value || '').toLowerCase().includes(type))) {
        ...
        if (targetHp <= 20) {
            ...
            game.Gametime.doIn({ hours: 1 }, async () => {
                game.cub.removeCondition(["Blinded", "Deafened", "Stunned"], target) // Not sure if this is better than the effect approach in my other macros, but it's nice not to need to use a hook
            });
        } else if (targetHp <= 40) {
