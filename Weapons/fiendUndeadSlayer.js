...
if (args[0].tag === 'OnUse') {
    ...
    const isFiendOrUndead = ['fiend', 'undead'].some(type => (target.actor.data.data.details?.type?.value || '').toLowerCase().includes(type))

    if (isFiendOrUndead) {
        const damageRoll = await new Roll("2d6").roll({ async: true });
        const damageType = 'radiant';
        new MidiQOL.DamageOnlyWorkflow(source, sourceToken, damageRoll.total, damageType, [target], damageRoll, {
